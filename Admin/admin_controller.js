...
const LoginAdmin = async (req, res, next) => {
  ...
  try {
    ...
    let result = await Model.getAllData(
      '`id`, `username`, `mobile`, `alternate_mobile`, `email_id`, `profile_dp`, `country`, `state`, `userType`',
      `tbl_user_web`,
    ...
    if (user) {
      ...
      let ChangeStatus = await Model.updateMaster(`tbl_user_web`, user.id, { login_status: 1 });
      const token = generateAuthToken();
      const response = await Model.addMaster('tbl_token', { token, user_id: user.id });
      if (response) {
        user.token = token;
        endConnection();
